...
<div id="wrap1">
    ...
    <!-- 为 ECharts 准备一个具备大小（宽高）的 DOM -->
    <div id="illumination" style="width: 280px;height:280px;"></div>
    <script type="text/javascript">
        // 基于准备好的dom，初始化ECharts实例
        var myChart4 = echarts.init(document.getElementById('illumination'));
        ...
        $(document).ready(function () {
            setInterval("start4()", 3000);
        });

        function start4() {
            $.get("http://localhost:8080/trio/getjson", "total=1", function (data, status) {
                //$("#mainContent").html(data.rows[0].carbon_dioxide);//要刷新的div
                option4.series[0].data[0].value = data.rows[0].illumination;
    ...
    <!-- 为 ECharts 准备一个具备大小（宽高）的 DOM -->
    <div id="gas" style="width: 280px;height:280px;"></div>
    <script type="text/javascript">
        // 基于准备好的dom，初始化ECharts实例
        var myChart5 = echarts.init(document.getElementById('gas'));
        ...
        $(document).ready(function () {
            setInterval("start5()", 3000);
        });

        function start5() {
            $.get("http://localhost:8080/trio/getjson", "total=1", function (data, status) {
                //$("#mainContent").html(data.rows[0].carbon_dioxide);//要刷新的div
                option5.series[0].data[0].value = data.rows[0].gas;
